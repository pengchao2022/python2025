...
def load_data():
    global students
    if os.path.exists('allen_stu2.json'):
        try:
            with open('allen_stu2.json', 'r') as f:
...
def save_data():
    try:
        with open('allen_stu2.json', 'w') as f:
...
if __name__ == "__main__":
    load_data() #启动时加载历史数据
    while True:
        student = input_stu_info()
        if student is None:
            print("感谢使用，再见！")
            break
        students.append(student)
        print(f"录入完毕！共录入{len(students)}条姓名") 
        save_data() 
